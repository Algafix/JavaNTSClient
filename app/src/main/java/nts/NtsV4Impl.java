 ...
 */
public class NtsV4Impl implements NtpV3Packet {
...
//    private static final int MESSAGE_DIGEST = 54; /* len 16 bytes */
    ...
    /** Creates a new instance of NtsV4Impl */
    public NtsV4Impl() {
    }
    ...
    public void addUniqueIdentifierEF(byte[] unique_identifier_body) {
        ...
        extensionFields.add(new NTSExtensionField(FieldType.UNIQUE_IDENTIFIER, unique_identifier_body));
    }
    ...
    public void addCookieEF(byte[] cookie_body) {
        extensionFields.add(new NTSExtensionField(FieldType.NTS_COOKIE, cookie_body));
    }
    ...
    /**
     * Returns the current NTSv4 packet as byte array.
     * Concatenates the normal NTP packet with the available extension fields.
     *
     * @return the current NTSv4 packet as byte array.
     */
    private byte[] getNTSv4Packet() {
        int extLen = 0;
        for (NTSExtensionField ef : extensionFields) {
            extLen += ef.getFieldLength();
        }
        byte[] currentBuffer = new byte[buf.length + extLen];
        System.arraycopy(buf, 0, currentBuffer, 0, buf.length);
        int offset = buf.length;
        for (NTSExtensionField ef : extensionFields) {
            byte[] fieldBytes = ef.toByteArray();
            System.arraycopy(fieldBytes, 0, currentBuffer, offset, fieldBytes.length);
     ...
     * <code>NtsV4Impl</code> object that contains the same values as this object.
     *
    ...
    public boolean equals(final Object obj) {
        ...
        final NtsV4Impl other = (NtsV4Impl) obj;
    ...
    public synchronized DatagramPacket getDatagramPacket() {
        if (dp == null) {
            if (ntsv4_buf == null) {
                // This is the case if we have not called prepareAuthAndEncEF() yet
                // We assume it is a standard NTP packet for now
                dp = new DatagramPacket(buf, buf.length);
            } else {
                dp = new DatagramPacket(ntsv4_buf, ntsv4_buf.length);
            }
            dp.setPort(NTP_PORT);
        }
        return dp;
    ...
    private void setTimestamp(final int index, final TimeStamp t) {
        ...
        for (int i = 7; i >= 0; i--) {
            ntsv4_buf[index + i] = (byte) (ntpTime & 0xFF);
